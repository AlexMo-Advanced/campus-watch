...
function loadBleManagerClass() {
  ...
  try {
    ...
    try {
      BlePeripheralClass = require('react-native-ble-peripheral').default || require('react-native-ble-peripheral');
    } catch {
      BlePeripheralClass = null;
    }
  } catch {
...
async function requestAndroidBlePermissions() {
  ...
  if (typeof sdkVersion === 'number' && sdkVersion >= 31) {
    try {
...
class BleProximityService {
  ...
  async start() {
    ...
    // Request Android 12+ permissions before anything else
    this.permissionsGranted = await requestAndroidBlePermissions();

    this.running = true;
    await this.rotateToken();
    await this.startScanning();

    // Listen for Bluetooth state changes so scanning restarts when BT is re-enabled
    const manager = this.getManager();
    if (manager && !this.stateSubscription) {
      this.stateSubscription = manager.onStateChange(async (state) => {
        if (!this.running) return;
        if (state === 'PoweredOn') {
          await this.startScanning();
        } else {
  ...
  async rotateToken() {
    ...
    // Heartbeat to server so the token is registered in ble_tokens table
    await sendProximityHeartbeat(next);
    // Update the advertised local name so nearby scanners can detect us
    this._advertiseLocalName(next);
  }

  /**
   * Attempt to advertise our current token via the BLE local name.
   */
  _advertiseLocalName(token) {
    if (!BlePeripheralClass) return;
    try {
      BlePeripheralClass.setName('cw:' + token);
      const promise = BlePeripheralClass.start();
  ...
  async startScanning() {
    ...
    try {
      ...
      manager.startDeviceScan(null, { allowDuplicates: true }, (error, device) => {
        if (error || !device || !this.running) {
          if (error) this.scanning = false;
          return;
        }
  ...
  /** Returns whether Android BLE permissions were granted at last start(). */
