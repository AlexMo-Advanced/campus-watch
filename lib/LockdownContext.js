...
export function LockdownProvider({ children }) {
  ...
  const triggerLockdown = useCallback((report, source = 'server', reportNotificationId = null) => {
    if (!lockdownEnabledRef.current || !report?.id) return;
    const enriched = { ...report, _source: source, _reportNotificationId: reportNotificationId };
    if (reportingActiveRef.current) {
      pendingRef.current = enriched;
      return;
    }
    scheduleAlert(enriched);
  }, [scheduleAlert]);
