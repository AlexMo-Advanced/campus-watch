 ...
 */
export async function generateCampusReport(reports, newCount, nearbyCount) {
  ...
  return chat([
    {
      role: 'system',
      content: 'You are a campus safety AI generating concise daily briefings for students on CampusWatch.',
    },
    {
      role: 'user',
...
Write a 2-3 sentence campus safety briefing. Be factual, calm, and helpful. Mention the most important trend or concern. Do NOT use bullet points or markdown. Plain prose only.`,
    },
  ]);
}
 ...
 * @returns {Promise<string>}
 */
export async function askCampusAssistant(history, reportContext = '') {
  const client = await getClient();
...
Be warm, concise, and reassuring. If someone describes an emergency, always tell them to call 911 first.
