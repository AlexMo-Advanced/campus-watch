...
export function NotificationProvider({ children, userId }) {
  ...
  // Case: user tapped the crisis push notification (existing path — unchanged)
  useEffect(() => {
    if (
      ...
      if (data?.lockdown && data?.reportId) {
        supabase
          ...
          .then(({ data: report }) => {
            if (report) {
              triggerLockdown(report, 'background_push', data.reportNotificationId ?? null);
            }
          })
          .catch(() => {});
      }
  ...
  // Case 1: crisis push arrives while app is already open (foreground)
  useEffect(() => {
    ...
    const foregroundSub = Notifications.addNotificationReceivedListener((notification) => {
      ...
      if (data?.lockdown && data?.reportId) {
        supabase
          ...
          .then(({ data: report }) => {
            if (report) {
              triggerLockdown(report, 'foreground_push', data.reportNotificationId ?? null);
            }
          })
          .catch(() => {});
      }
    ...
    const priorityChannel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (isBleCrisis) {
            triggerLockdown(report, 'ble', row.id);
          }
