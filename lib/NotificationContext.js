...
export function NotificationProvider({ children, userId }) {
  ...
  const checkNearbyReports = useCallback(async (reports) => {
    ...
    try {
      ...
      const active = reports.filter((r) => r.status !== 'resolved' && r.latitude && r.longitude);
  ...
  useEffect(() => {
    ...
    const newReportChannel = supabase
      .channel('notification-new-reports')
      .on(
        'postgres_changes',
        ...
        async (payload) => {
          ...
          const uid = userIdRef.current;
          if (!uid || report.user_id === uid) return;
          await push({
            type: NOTIFICATION_TYPES.NEARBY_SITUATION,
            title: 'New campus alert',
            body: `"${report.title}" was just reported${report.location ? ` at ${report.location}` : ''}.`,
            dedupeKey: `new_report_${report.id}`,
            metadata: { reportId: report.id },
          });
          await firePushNotification(
            'New campus alert',
            `"${report.title}" was just reported${report.location ? ` at ${report.location}` : ''}.`,
            { reportId: report.id },
            NOTIFICATION_TYPES.NEARBY_SITUATION
          );
        }
    ...
    return () => {
      supabase.removeChannel(newReportChannel);
