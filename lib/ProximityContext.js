  ...
  const startService = useCallback(async () => {
    ...
    if (BLE_NATIVE_ENABLED) {
      try {
        ...
        // Reflect runtime permission result after start
        if (typeof serviceRef.current.isBlePermissionsGranted === 'function') {
          setBlePermissionsGranted(serviceRef.current.isBlePermissionsGranted());
        }
      } catch {
        // fail closed
      }
